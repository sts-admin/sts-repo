(function($){
    ...
    var methods = {
        ...
        openModal: function(url, options){
			...
			// necessary becuase reinitializing of the modal calling it every time
			if(typeof(pre_exist) === 'undefined')
			{
				if (url.toLowerCase().indexOf("/t/") >= 0)
				{ 
					$('#prev_url_to').val(window.location.href);
        ...
        closeModal: function () {
			...
			if( set_history == 1 )
			{
				window.history.pushState("object or string", "Title", $('#prev_url_to').val()); 
				//window.history.back();
			}
